>>>>>>>>>>main>>>>PREFIX>PREFIX_UPPER>SERVICE_NAME
...
void epa_kernel_set_debug_callback(EpaKernel *k, %PREFIX%KernelDbgCallback cb, void *cb_user);
}
...
%SERVICE_NAME%::%SERVICE_NAME%() : JsonRPCService("epa"), next_event_id(1) {}

%SERVICE_NAME%::~%SERVICE_NAME%() {}

void %SERVICE_NAME%::onKernelDebug(void *cb_user, int kind, uint8_t wid, uint32_t code, const ::%PREFIX%DbgEip *at, const char *msg) {
    %SERVICE_NAME% *self = (%SERVICE_NAME%*)cb_user;
    if (!self) return;
...
void %SERVICE_NAME%::ensureDebugCallbackInstalled() {
    if (host.rawKernel()) {
        epa_kernel_set_debug_callback(host.rawKernel(), (%PREFIX%KernelDbgCallback)onKernelDebug, this);
    }
}

void %SERVICE_NAME%::pushEvent(const String &kind, uint32_t wid, uint32_t code, const %PREFIX%DbgEip *at, const String &message) {
    DebugEvent event;
    event.kind = kind;
    event.wid = wid;
    event.code = code;
...
void %SERVICE_NAME%::pushLogEvent(const String &message) {
    if (!message.length()) return;
    pushEvent(String("log"), 0, 0, NULL, message);
}

bool %SERVICE_NAME%::parseStringField(const String &json, const String &field, String &out_value) const {
    return JsonRPCCodec::getStringField(json, field, out_value);
}

bool %SERVICE_NAME%::parseUintField(const String &json, const String &field, uint32_t default_value, uint32_t *out_value) const {
    String raw;
    String text(json);
...
bool %SERVICE_NAME%::parseBoolField(const String &json, const String &field, bool default_value, bool *out_value) const {
    String raw;
    String text(json);
    if (JsonRPCCodec::getStringField(text, field, raw)) {
        raw = raw.trim();
...
bool %SERVICE_NAME%::parseHexBytes(const String &hex, std::vector<unsigned char> &bytes) const {
    String text(hex);
    text = text.trim();
    bytes.clear();
...
bool %SERVICE_NAME%::hasBreakpointHit(uint32_t *out_wid, Breakpoint *out_breakpoint) const {
    ...
    for (i = 0; i < breakpoints.size(); i++) {
        ...
        if (%PREFIX%_epa_debug_any_worker_at(kernel, bp.block_type, bp.block_id, bp.rel_pc, &wid)) {
...
bool %SERVICE_NAME%::runTicks(uint32_t tick_count, bool stop_on_breakpoint, String &stop_reason, uint32_t &ticks_ran, String &error_message) {
    EpaKernel *kernel = host.rawKernel();
    ...
    for (;;) {
        ...
        if (stop_on_breakpoint) {
            ...
            if (hasBreakpointHit(&wid, &bp)) {
                %PREFIX%DbgEip at; at.block_type = bp.block_type; at.block_id = bp.block_id; at.rel_pc = bp.rel_pc;
...
String %SERVICE_NAME%::buildSnapshotJson() const {
    String result("{");
    %PREFIX%EpaDebugKernelSnapshot kernel_snapshot;
    %PREFIX%EpaDebugWorkerSnapshot workers[%PREFIX_UPPER%_EPA_DEBUG_MAX_WORKERS];
    size_t worker_count = 0;
    memset(&kernel_snapshot, 0, sizeof(kernel_snapshot));
    memset(workers, 0, sizeof(workers));
    if (host.rawKernel()) {
        %PREFIX%_epa_debug_capture_kernel(host.rawKernel(), &kernel_snapshot);
        worker_count = %PREFIX%_epa_debug_capture_workers(host.rawKernel(), workers, %PREFIX_UPPER%_EPA_DEBUG_MAX_WORKERS);
    }
    ...
    for (size_t i = 0; i < worker_count; i++) {
        ...
        for (uint32_t j = 0; j < %PREFIX_UPPER%_EPA_DEBUG_LOCALS; j++) { if (j) result += String(","); result += String((int)workers[i].locals[j]); }
        result += String("]");
...
String %SERVICE_NAME%::buildEventsJson(bool clear_after_read) {
...
String %SERVICE_NAME%::buildBreakpointJson() const {
...
bool %SERVICE_NAME%::call(const String &method, const String &params_json, String &result_json, String &error_code, String &error_message) {
