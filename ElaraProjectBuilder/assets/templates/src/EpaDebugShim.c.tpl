...
size_t %PREFIX%_epa_debug_capture_workers(EpaKernel *kernel, %PREFIX%EpaDebugWorkerSnapshot *out_workers, size_t max_workers) {
  ...
  for (wid = 0; wid < EPA_MAX_WORKERS && count < max_workers; wid++) {
    ...
    %PREFIX%EpaDebugWorkerSnapshot *dst;
    size_t i;
    ...
    dst->stack_preview_count = (uint32_t)((w->vm.stack.sp < %PREFIX_UPPER%_EPA_DEBUG_STACK_PREVIEW) ? w->vm.stack.sp : %PREFIX_UPPER%_EPA_DEBUG_STACK_PREVIEW);
