...
function int fs_boot_walk_phase_superblock() {
  return 10;
}

function int fs_boot_walk_phase_kernel_inode() {
  return 11;
}
...
function int fs_result_payload_u32_le(BlockIoReadResult result, int payload_offset) {
  int base = payload_offset;
  return fs_result_payload_u8(result, base)
    + (fs_result_payload_u8(result, base + 1) * 256)
    + (fs_result_payload_u8(result, base + 2) * 65536)
    + (fs_result_payload_u8(result, base + 3) * 16777216);
}
...
worker filesystem_boot_assets_ingress(FileSystemBootAssetRequest request) {
  ...
  if (opcode == filesystem_boot_assets_opcode_kick_ext4_walk()) {
    ...
    block_request.lba = 2;
    block_request.block_count = 2;
    block_request.phase = fs_boot_walk_phase_superblock();
    block_request.target_inode = target_inode;
    block_request.block_size = 0;
    block_request.inode_table_block = 0;
    frame_begin(7357000, drive_id, target_inode, 2, 2);
    frame_commit();
    far_signal("elara.os.block_io", block_io_ingress, block_request);
    retire_worker();
  }
...
worker filesystem_block_read_result_ingress(BlockIoReadResult result) {
  ...
  int sb_magic = 0;
  int sb_log_block_size = 0;
  int sb_inodes_per_group = 0;
  int sb_block_size = 0;
  ...
  frame_begin(7357099, ctx_phase, ctx_target_inode, result.byte_count, result.drive_id);
  frame_commit();

  if (ctx_phase == fs_boot_walk_phase_superblock()) {
    if (result.byte_count > 63) {
      sb_magic = fs_result_payload_u16_le(result, 56);
      sb_log_block_size = fs_result_payload_u32_le(result, 24);
      sb_inodes_per_group = fs_result_payload_u32_le(result, 40);
      sb_block_size = 1024;
      if (sb_log_block_size == 1) {
        sb_block_size = 2048;
      }
      if (sb_log_block_size == 2) {
        sb_block_size = 4096;
      }
      group_desc_lba = sb_block_size / 512;
      if (sb_block_size == 1024) {
        group_desc_lba = 4;
      }
      group_desc_block_count = sb_block_size / 512;
      ...
      frame_begin(7357013, result.drive_id, sb_magic, sb_block_size, sb_log_block_size);
      frame_commit();
      if (sb_magic == 61267) {
        block_request.opcode = block_io_opcode_read_blocks();
        ...
        block_request.block_size = sb_block_size;
        block_request.inode_table_block = 0;
        frame_begin(7357001, result.drive_id, ctx_target_inode, group_desc_lba, group_desc_block_count);
        frame_commit();
        far_signal("elara.os.block_io", block_io_ingress, block_request);
      }
    }
  ...
  if (ctx_phase == fs_boot_walk_phase_elara_directory()) {
    ...
    while (dir_offset < result.byte_count) {
      ...
      if (entry_candidate != 0) {
        ...
        frame_commit();
        far_signal("elara.os.filesystem", register_directory_entry, directory_entry);

        if (entry_candidate == fs_path_hash_epa_kernel_bin()) {
          inode_index = entry_inode - 1;
          inode_byte_offset = (ctx_inode_table_block * ctx_block_size) + (inode_index * 256);
          inode_lba = inode_byte_offset / 512;
          inode_block_count = ctx_block_size / 512;
          ...
          block_request.lba = inode_lba;
          block_request.block_count = inode_block_count;
          block_request.phase = fs_boot_walk_phase_kernel_inode();
          block_request.target_inode = entry_inode;
          block_request.block_size = ctx_block_size;
          block_request.inode_table_block = ctx_inode_table_block;
          far_signal("elara.os.block_io", block_io_ingress, block_request);
  ...
  if (ctx_phase == fs_boot_walk_phase_kernel_inode()) {
    if (ctx_block_size > 0) {
      inode_index = ctx_target_inode - 1;
      inode_byte_offset = (ctx_inode_table_block * ctx_block_size) + (inode_index * 256);
      inode_lba = inode_byte_offset / 512;
      inode_payload_offset = inode_byte_offset - (inode_lba * 512);
      ctx_arg0 = fs_result_payload_u32_le(result, inode_payload_offset + 4);
      ctx_arg1 = fs_result_payload_u32_le(result, inode_payload_offset + 60);
      frame_begin(7357031, result.drive_id, ctx_arg0, ctx_arg1, ctx_block_size);
      frame_commit();
    }
  }
...
worker register_directory_entry(Ext4DirectoryEntryRegistration entry) {
  ...
  dynamic dir_entry_nodes(FileNodeState, 8, 128, 16);
