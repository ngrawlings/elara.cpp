...
void epa_kernel_set_debug_callback(EpaKernel *k, EpaSignalLabKernelDbgCallback cb, void *cb_user);
}
...
EpaSignalLabEpaDebugService::EpaSignalLabEpaDebugService() : JsonRPCService("epa"), next_event_id(1) {}

EpaSignalLabEpaDebugService::~EpaSignalLabEpaDebugService() {}

void EpaSignalLabEpaDebugService::onKernelDebug(void *cb_user, int kind, uint8_t wid, uint32_t code, const ::EpaSignalLabDbgEip *at, const char *msg) {
    EpaSignalLabEpaDebugService *self = (EpaSignalLabEpaDebugService*)cb_user;
    if (!self) return;
...
void EpaSignalLabEpaDebugService::ensureDebugCallbackInstalled() {
    if (host.rawKernel()) {
        epa_kernel_set_debug_callback(host.rawKernel(), (EpaSignalLabKernelDbgCallback)onKernelDebug, this);
    }
}

void EpaSignalLabEpaDebugService::pushEvent(const String &kind, uint32_t wid, uint32_t code, const EpaSignalLabDbgEip *at, const String &message) {
    DebugEvent event;
    event.kind = kind;
    event.wid = wid;
    event.code = code;
...
void EpaSignalLabEpaDebugService::pushLogEvent(const String &message) {
    if (!message.length()) return;
    pushEvent(String("log"), 0, 0, NULL, message);
}

bool EpaSignalLabEpaDebugService::parseStringField(const String &json, const String &field, String &out_value) const {
    return JsonRPCCodec::getStringField(json, field, out_value);
}

bool EpaSignalLabEpaDebugService::parseUintField(const String &json, const String &field, uint32_t default_value, uint32_t *out_value) const {
    String raw;
    String text(json);
...
bool EpaSignalLabEpaDebugService::parseBoolField(const String &json, const String &field, bool default_value, bool *out_value) const {
    String raw;
    String text(json);
    if (JsonRPCCodec::getStringField(text, field, raw)) {
        raw = raw.trim();
...
bool EpaSignalLabEpaDebugService::parseHexBytes(const String &hex, std::vector<unsigned char> &bytes) const {
    String text(hex);
    text = text.trim();
    bytes.clear();
...
bool EpaSignalLabEpaDebugService::hasBreakpointHit(uint32_t *out_wid, Breakpoint *out_breakpoint) const {
    ...
    for (i = 0; i < breakpoints.size(); i++) {
        ...
        if (EpaSignalLab_epa_debug_any_worker_at(kernel, bp.block_type, bp.block_id, bp.rel_pc, &wid)) {
...
bool EpaSignalLabEpaDebugService::runTicks(uint32_t tick_count, bool stop_on_breakpoint, String &stop_reason, uint32_t &ticks_ran, String &error_message) {
    EpaKernel *kernel = host.rawKernel();
    ...
    for (;;) {
        ...
        if (stop_on_breakpoint) {
            ...
            if (hasBreakpointHit(&wid, &bp)) {
                EpaSignalLabDbgEip at; at.block_type = bp.block_type; at.block_id = bp.block_id; at.rel_pc = bp.rel_pc;
...
String EpaSignalLabEpaDebugService::buildSnapshotJson() const {
    String result("{");
    EpaSignalLabEpaDebugKernelSnapshot kernel_snapshot;
    EpaSignalLabEpaDebugWorkerSnapshot workers[EPASIGNALLAB_EPA_DEBUG_MAX_WORKERS];
    size_t worker_count = 0;
    memset(&kernel_snapshot, 0, sizeof(kernel_snapshot));
    memset(workers, 0, sizeof(workers));
    if (host.rawKernel()) {
        EpaSignalLab_epa_debug_capture_kernel(host.rawKernel(), &kernel_snapshot);
        worker_count = EpaSignalLab_epa_debug_capture_workers(host.rawKernel(), workers, EPASIGNALLAB_EPA_DEBUG_MAX_WORKERS);
    }
    ...
    for (size_t i = 0; i < worker_count; i++) {
        ...
        for (uint32_t j = 0; j < EPASIGNALLAB_EPA_DEBUG_LOCALS; j++) { if (j) result += String(","); result += String((int)workers[i].locals[j]); }
        result += String("]");
...
String EpaSignalLabEpaDebugService::buildEventsJson(bool clear_after_read) {
...
String EpaSignalLabEpaDebugService::buildBreakpointJson() const {
...
bool EpaSignalLabEpaDebugService::call(const String &method, const String &params_json, String &result_json, String &error_code, String &error_message) {
