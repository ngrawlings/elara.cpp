#ifndef EPASIGNALLABEPADEBUGSERVICE_H
#define EPASIGNALLABEPADEBUGSERVICE_H
...
#include "EpaSignalLabEpaDebugShim.h"
...
typedef void (*EpaSignalLabKernelDbgCallback)(void *cb_user, int kind, uint8_t wid, uint32_t code, const EpaSignalLabDbgEip *at, const char *msg);

namespace elara {

class EpaSignalLabEpaDebugService : public sockets::rpc::json::JsonRPCService {
public:
    EpaSignalLabEpaDebugService();
    virtual ~EpaSignalLabEpaDebugService();
...
private:
    ...
    EpaSignalLabEpaVmHost host;
    std::deque<DebugEvent> events;
    std::vector<Breakpoint> breakpoints;
    uint64_t next_event_id;

    static void onKernelDebug(void *cb_user, int kind, uint8_t wid, uint32_t code, const ::EpaSignalLabDbgEip *at, const char *msg);
    void ensureDebugCallbackInstalled();
    void pushEvent(const String &kind, uint32_t wid, uint32_t code, const EpaSignalLabDbgEip *at, const String &message);
