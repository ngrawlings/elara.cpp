...
size_t EpaSignalLab_epa_debug_capture_workers(EpaKernel *kernel, EpaSignalLabEpaDebugWorkerSnapshot *out_workers, size_t max_workers) {
  ...
  for (wid = 0; wid < EPA_MAX_WORKERS && count < max_workers; wid++) {
    ...
    EpaSignalLabEpaDebugWorkerSnapshot *dst;
    size_t i;
    ...
    dst->stack_preview_count = (uint32_t)((w->vm.stack.sp < EPASIGNALLAB_EPA_DEBUG_STACK_PREVIEW) ? w->vm.stack.sp : EPASIGNALLAB_EPA_DEBUG_STACK_PREVIEW);
