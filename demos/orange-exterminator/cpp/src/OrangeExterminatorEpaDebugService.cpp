...
void epa_kernel_set_debug_callback(EpaKernel *k, OrangeExterminatorKernelDbgCallback cb, void *cb_user);
}
...
OrangeExterminatorEpaDebugService::OrangeExterminatorEpaDebugService() : JsonRPCService("epa"), next_event_id(1) {}

OrangeExterminatorEpaDebugService::~OrangeExterminatorEpaDebugService() {}

void OrangeExterminatorEpaDebugService::onKernelDebug(void *cb_user, int kind, uint8_t wid, uint32_t code, const ::OrangeExterminatorDbgEip *at, const char *msg) {
    OrangeExterminatorEpaDebugService *self = (OrangeExterminatorEpaDebugService*)cb_user;
    if (!self) return;
...
void OrangeExterminatorEpaDebugService::ensureDebugCallbackInstalled() {
    if (host.rawKernel()) {
        epa_kernel_set_debug_callback(host.rawKernel(), (OrangeExterminatorKernelDbgCallback)onKernelDebug, this);
    }
}

void OrangeExterminatorEpaDebugService::pushEvent(const String &kind, uint32_t wid, uint32_t code, const OrangeExterminatorDbgEip *at, const String &message) {
    DebugEvent event;
    event.kind = kind;
    event.wid = wid;
    event.code = code;
...
void OrangeExterminatorEpaDebugService::pushLogEvent(const String &message) {
    if (!message.length()) return;
    pushEvent(String("log"), 0, 0, NULL, message);
}

bool OrangeExterminatorEpaDebugService::parseStringField(const String &json, const String &field, String &out_value) const {
    return JsonRPCCodec::getStringField(json, field, out_value);
}

bool OrangeExterminatorEpaDebugService::parseUintField(const String &json, const String &field, uint32_t default_value, uint32_t *out_value) const {
    String raw;
    String text(json);
...
bool OrangeExterminatorEpaDebugService::parseBoolField(const String &json, const String &field, bool default_value, bool *out_value) const {
    String raw;
    String text(json);
    if (JsonRPCCodec::getStringField(text, field, raw)) {
        raw = raw.trim();
...
bool OrangeExterminatorEpaDebugService::parseHexBytes(const String &hex, std::vector<unsigned char> &bytes) const {
    String text(hex);
    text = text.trim();
    bytes.clear();
...
bool OrangeExterminatorEpaDebugService::hasBreakpointHit(uint32_t *out_wid, Breakpoint *out_breakpoint) const {
    ...
    for (i = 0; i < breakpoints.size(); i++) {
        ...
        if (OrangeExterminator_epa_debug_any_worker_at(kernel, bp.block_type, bp.block_id, bp.rel_pc, &wid)) {
...
bool OrangeExterminatorEpaDebugService::runTicks(uint32_t tick_count, bool stop_on_breakpoint, String &stop_reason, uint32_t &ticks_ran, String &error_message) {
    EpaKernel *kernel = host.rawKernel();
    ...
    for (;;) {
        ...
        if (stop_on_breakpoint) {
            ...
            if (hasBreakpointHit(&wid, &bp)) {
                OrangeExterminatorDbgEip at; at.block_type = bp.block_type; at.block_id = bp.block_id; at.rel_pc = bp.rel_pc;
...
String OrangeExterminatorEpaDebugService::buildSnapshotJson() const {
    String result("{");
    ...
    if (host.rawKernel()) {
        OrangeExterminator_epa_debug_capture_kernel(host.rawKernel(), &kernel_snapshot);
        worker_count = OrangeExterminator_epa_debug_capture_workers(host.rawKernel(), workers, ORANGEEXTERMINATOR_EPA_DEBUG_MAX_WORKERS);
    }
    ...
    for (size_t i = 0; i < worker_count; i++) {
        ...
        for (uint32_t j = 0; j < ORANGEEXTERMINATOR_EPA_DEBUG_LOCALS; j++) { if (j) result += String(","); result += String((int)workers[i].locals[j]); }
        result += String("]");
...
String OrangeExterminatorEpaDebugService::buildEventsJson(bool clear_after_read) {
...
String OrangeExterminatorEpaDebugService::buildBreakpointJson() const {
...
bool OrangeExterminatorEpaDebugService::call(const String &method, const String &params_json, String &result_json, String &error_code, String &error_message) {
