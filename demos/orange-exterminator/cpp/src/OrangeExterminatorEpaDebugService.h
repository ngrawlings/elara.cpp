#ifndef ORANGEEXTERMINATOREPADEBUGSERVICE_H
#define ORANGEEXTERMINATOREPADEBUGSERVICE_H
...
typedef struct {
    ...
    uint16_t block_id;
    uint32_t rel_pc;
} OrangeExterminatorDbgEip;

typedef void (*OrangeExterminatorKernelDbgCallback)(void *cb_user, int kind, uint8_t wid, uint32_t code, const OrangeExterminatorDbgEip *at, const char *msg);

namespace elara {

class OrangeExterminatorEpaDebugService : public sockets::rpc::json::JsonRPCService {
public:
    OrangeExterminatorEpaDebugService();
    virtual ~OrangeExterminatorEpaDebugService();
...
private:
    ...
    OrangeExterminatorEpaVmHost host;
    std::deque<DebugEvent> events;
    std::vector<Breakpoint> breakpoints;
    uint64_t next_event_id;

    static void onKernelDebug(void *cb_user, int kind, uint8_t wid, uint32_t code, const ::OrangeExterminatorDbgEip *at, const char *msg);
    void ensureDebugCallbackInstalled();
    void pushEvent(const String &kind, uint32_t wid, uint32_t code, const OrangeExterminatorDbgEip *at, const String &message);
