...
if ! make -C "${FRAMEWORK_ROOT}/libElaraUI" rpc-server -j2 >/dev/null; then
  if [[ -x "${FRAMEWORK_ROOT}/libElaraUI/build/bin/elaraui-server" ]]; then
    echo "[orange-exterminator] warning: staged UI server copy failed, using fresh local UI server binary"
  else
    echo "[orange-exterminator] UI server build failed and no fresh local binary is available"
    exit 1
  fi
fi
...
echo "[orange-exterminator] building C++ host"
( cd "${CPP_DIR}" && ./build.sh >/dev/null )

echo "[orange-exterminator] starting UI head on ${UI_HOST}:${UI_PORT}"
...
if ! kill -0 "${EPA_DEBUG_PID}" 2>/dev/null; then
  EPA_DEBUG_PID=""
  echo "[orange-exterminator] warning: EPA debug RPC failed to start. See ${EPA_DEBUG_LOG}"
else
  EPA_DEBUG_OK=1
fi
