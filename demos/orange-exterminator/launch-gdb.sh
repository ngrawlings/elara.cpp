...
APP_LOG="${ARTIFACT_DIR}/orange-exterminator-${UI_PORT}.log"
...
if [[ -x "${ROOT_DIR}/kill-all.sh" ]]; then
  echo "[orange-exterminator] clearing existing related processes"
  "${ROOT_DIR}/kill-all.sh" >/dev/null 2>&1 || true
fi

echo "[orange-exterminator] rebuilding EPA runtime"
...
if ! make -C "${FRAMEWORK_ROOT}/libElaraUI" rpc-server -j2 >/dev/null; then
  if [[ -x "${FRAMEWORK_ROOT}/libElaraUI/build/bin/elaraui-server" ]]; then
    echo "[orange-exterminator] warning: UI server copy failed, using existing binary"
  else
    echo "[orange-exterminator] UI server build failed and no binary available"
    exit 1
  fi
fi

echo "[orange-exterminator] building EPA bundle -> ${BUILD_DIR}/epa.bin"
...
echo "[orange-exterminator] building C++ host (debug)"
...
if [[ -x "${LOCAL_FRESH_SERVER}" ]]; then
  ...
  echo "[orange-exterminator] cannot find elaraui-server binary"
  exit 1
fi

echo "[orange-exterminator] UI server binary: ${ELARA_UI_SERVER}"

if [[ "${GDB_INTERACTIVE}" == "1" ]]; then
  echo "[orange-exterminator] starting UI head under interactive GDB in xterm"
  echo "[orange-exterminator] GDB tip: run, then bt/bt full after crash"
  ( xterm -e "gdb --args ${ELARA_UI_SERVER} --port ${UI_PORT}" >"${UI_LOG}" 2>&1 ) &
else
  echo "[orange-exterminator] starting UI head under GDB (batch, crash log -> ${GDB_LOG})"
...
echo "[orange-exterminator] starting EPA debug RPC on ${EPA_DEBUG_HOST}:${EPA_DEBUG_PORT}"
( cd "${CPP_DIR}" && ./run-epa-debug.sh "${EPA_DEBUG_PORT}" "${EPA_DEBUG_HOST}" >"${EPA_DEBUG_LOG}" 2>&1 ) &
EPA_DEBUG_PID="$!"
sleep 1
if ! kill -0 "${EPA_DEBUG_PID}" 2>/dev/null; then
  EPA_DEBUG_PID=""
  echo "[orange-exterminator] warning: EPA debug RPC failed to start"
else
  EPA_DEBUG_OK=1
fi

echo "[orange-exterminator] logs:"
...
echo "[orange-exterminator] launching client"
cd "${CPP_DIR}"
./build/orange-exterminator "${UI_HOST}" "${UI_PORT}" 2>&1 | tee "${APP_LOG}"
