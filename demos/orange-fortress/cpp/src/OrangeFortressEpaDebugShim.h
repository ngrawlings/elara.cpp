#ifndef ORANGEFORTRESSEPADEBUGSHIM_H
#define ORANGEFORTRESSEPADEBUGSHIM_H
...
#define ORANGEFORTRESS_EPA_DEBUG_MAX_WORKERS 256
#define ORANGEFORTRESS_EPA_DEBUG_STACK_PREVIEW 8
#define ORANGEFORTRESS_EPA_DEBUG_LOCALS 32

typedef struct {
  uint8_t block_type;
  uint16_t block_id;
  uint32_t rel_pc;
} OrangeFortressEpaDebugEip;
...
typedef struct {
  ...
  uint32_t stack_preview[ORANGEFORTRESS_EPA_DEBUG_STACK_PREVIEW];
  uint32_t inq_count;
  uint32_t outq_count;
  int32_t locals[ORANGEFORTRESS_EPA_DEBUG_LOCALS];
  uint32_t lbytes_top;
  uint32_t lbytes_cap;
  uint32_t lscope_depth;
  uint64_t current_ghs;
  OrangeFortressEpaDebugEip eip;
} OrangeFortressEpaDebugWorkerSnapshot;
...
} OrangeFortressEpaDebugKernelSnapshot;
