...
OrangeFortressEpaVmHost::OrangeFortressEpaVmHost()
    : kernel(NULL),
      module(NULL) {
}
...
void OrangeFortressEpaVmHost::setError(const char *prefix, const char *detail) {
...
bool OrangeFortressEpaVmHost::create() {
...
void OrangeFortressEpaVmHost::destroy() {
    if (module) {
...
bool OrangeFortressEpaVmHost::setKernelId(const String &kernel_id) {
    char err[EPA_MAX_ERR];
    if (!kernel && !create()) {
        return false;
    }
...
bool OrangeFortressEpaVmHost::loadAsmPath(const String &asm_path) {
    char err[EPA_MAX_ERR];
    if (!kernel && !create()) {
        return false;
    }
...
bool OrangeFortressEpaVmHost::loadBlob(const uint8_t *blob, size_t blob_len) {
    char err[EPA_MAX_ERR];
    if (!kernel && !create()) {
        return false;
    }
...
bool OrangeFortressEpaVmHost::loadBundlePath(const String &bundle_path) {
    char err[EPA_MAX_ERR];
...
bool OrangeFortressEpaVmHost::ingressPush(uint32_t wid, const void *data, uint32_t len) {
    if (!kernel) {
...
bool OrangeFortressEpaVmHost::ingressPushTagged(uint32_t wid, uint32_t tag, const void *data, uint32_t len) {
    if (!kernel) {
...
bool OrangeFortressEpaVmHost::ingressPushToKernel(size_t index, uint32_t wid, const void *data, uint32_t len) {
...
bool OrangeFortressEpaVmHost::ingressPushTaggedToKernel(size_t index, uint32_t wid, uint32_t tag, const void *data, uint32_t len) {
    EpaKernel *target = rawKernelAt(index);
    if (!target) {
...
bool OrangeFortressEpaVmHost::run(uint32_t max_ticks, bool debug) {
    char err[EPA_MAX_ERR];
    if (!kernel) {
...
const uint8_t *OrangeFortressEpaVmHost::resultData(size_t *out_len) const {
    if (out_len) *out_len = 0;
    error_text = String("epa_kernel_get_result unavailable: the current runtime bridge is module/status driven");
    return NULL;
}

EpaKernel *OrangeFortressEpaVmHost::rawKernel() const {
    return kernel;
}

EpaKernel *OrangeFortressEpaVmHost::rawKernelAt(size_t index) const {
    if (!module) {
        return NULL;
    }
...
size_t OrangeFortressEpaVmHost::kernelCount() const {
    if (!module) {
        return 0;
    }
...
String OrangeFortressEpaVmHost::kernelPathId(size_t index) const {
    const char *path_id = module ? epa_kernel_module_path_id(module, index) : NULL;
    return path_id ? String(path_id) : String();
}

String OrangeFortressEpaVmHost::kernelStatus(size_t index) const {
    if (!module) {
        return String();
    }
...
String OrangeFortressEpaVmHost::kernelError(size_t index) const {
    const char *error = module ? epa_kernel_module_kernel_error(module, index) : NULL;
    return error ? String(error) : String();
}

uint32_t OrangeFortressEpaVmHost::kernelThreadCount(size_t index) const {
    if (!module) {
        return 0;
    }
...
int OrangeFortressEpaVmHost::findKernelIndex(const String &path_id) const {
    if (!module) {
        return -1;
    }
...
bool OrangeFortressEpaVmHost::addKernelThreads(size_t index, uint32_t add_count) {
    char err[EPA_MAX_ERR];
    if (!module) {
...
bool OrangeFortressEpaVmHost::startAllKernels() {
    char err[EPA_MAX_ERR];
    if (!module) {
...
bool OrangeFortressEpaVmHost::stopAllKernels() {
    char err[EPA_MAX_ERR];
    if (!module) {
        return true;
    }
...
bool OrangeFortressEpaVmHost::isReady() const {
    return kernel != NULL || module != NULL;
}

const String &OrangeFortressEpaVmHost::lastError() const {
    return error_text;
}
