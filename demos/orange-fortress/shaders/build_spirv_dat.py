...
def build() -> bytes:
    ...
    cb_new = tid()
    depth_new = tid()
    s.emit(OP["Phi"], t_float, cr_new, cr_tri, l_tri_merge, cr_after2, l_op2_merge)
    s.emit(OP["Phi"], t_float, cg_new, cg_tri, l_tri_merge, cg_after2, l_op2_merge)
    s.emit(OP["Phi"], t_float, cb_new, cb_tri, l_tri_merge, cb_after2, l_op2_merge)
    s.emit(OP["Phi"], t_float, depth_new, depth_tri, l_tri_merge, depth_after2, l_op2_merge)
