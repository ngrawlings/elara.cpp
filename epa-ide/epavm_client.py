"""TCP BRPC client for epavm (port 18878).
...
class EpaDbgClient:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = DEFAULT_PORT):
        ...
        # epavm is request/response over one ordered TCP stream. The full
    ...
    def connect_retry(self, timeout: float = 10.0, interval: float = 0.25) -> None:
        ...
        while time.monotonic() < deadline:
            try:
                ...
                last_exc = exc
                time.sleep(interval)
        raise TimeoutError(f"epavm not reachable after {timeout}s: {last_exc}")
    ...
    def _recv_raw(self, timeout: float = 30.0) -> bytes:
        ...
        try:
            ...
            if length > 64 * 1024 * 1024:
                raise RuntimeError(f"epavm frame too large: {length} bytes")
