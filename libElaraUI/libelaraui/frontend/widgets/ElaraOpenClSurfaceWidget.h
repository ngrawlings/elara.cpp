#ifndef ELARA_OPENCL_SURFACE_WIDGET_H
#define ELARA_OPENCL_SURFACE_WIDGET_H
...
class ElaraOpenClSurfaceCommand {
public:
    enum Type {
        CLEAR,
        RECT,
    ...
    ElaraOpenClSurfaceCommand(Type command_type);
};

class ElaraOpenClSurfaceWidget : public ElaraCanvasWidget {
private:
    class OpenClRuntime;

    Array< Ref<ElaraOpenClSurfaceCommand> > commands;
    String backend_id;
    String kernel_name;
    String overlay_text;
    ...
    bool renderOpenCl(int pixel_width, int pixel_height);
    void drawEmptyState(ElaraDrawContext* ctx);

public:
    ElaraOpenClSurfaceWidget(
        ElaraWidgetRegister* root_widget,
        ElaraWidgetHandle widget_handle
    );

    virtual ~ElaraOpenClSurfaceWidget();
