...
void EpaDebugSmokeEpaDebugService::ensureDebugCallbackInstalled() {
    if (host.rawKernel()) {
        epa_kernel_set_debug_callback(host.rawKernel(), onKernelDebug, this);
    }
}
...
bool EpaDebugSmokeEpaDebugService::parseUintField(const String &json, const String &field, uint32_t default_value, uint32_t *out_value) const {
    String raw;
    if (JsonRPCCodec::getStringField(json, field, raw)) {
    ...
    start = json.indexOf(String(":"), start);
    if (start < 0) { if (out_value) *out_value = default_value; return false; }
    start += 1;
    while (start < json.length() && isspace(json.operator char *()[start])) start++;
    int end = start;
    while (end < json.length() && (isdigit(json.operator char *()[end]) || json.operator char *()[end] == 'x' || json.operator char *()[end] == 'X' || (json.operator char *()[end] >= 'a' && json.operator char *()[end] <= 'f') || (json.operator char *()[end] >= 'A' && json.operator char *()[end] <= 'F'))) end++;
    if (end <= start) { if (out_value) *out_value = default_value; return true; }
    raw = json.substr(start, end - start).trim();
