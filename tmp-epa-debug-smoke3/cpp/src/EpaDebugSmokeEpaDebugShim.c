...
size_t EpaDebugSmoke_epa_debug_capture_workers(EpaKernel *kernel, EpaDebugSmokeEpaDebugWorkerSnapshot *out_workers, size_t max_workers) {
  ...
  for (wid = 0; wid < EPA_MAX_WORKERS && count < max_workers; wid++) {
    ...
    EpaDebugSmokeEpaDebugWorkerSnapshot *dst;
    size_t i;
    ...
    dst->stack_preview_count = (uint32_t)((w->vm.stack.sp < EPADEBUGSMOKE_EPA_DEBUG_STACK_PREVIEW) ? w->vm.stack.sp : EPADEBUGSMOKE_EPA_DEBUG_STACK_PREVIEW);
    ...
    dst->current_ghs = ((uint64_t)w->current_ghs.gen << 32) | (uint64_t)w->current_ghs.idx;
