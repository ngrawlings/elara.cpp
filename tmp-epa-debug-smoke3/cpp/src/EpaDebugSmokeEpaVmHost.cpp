#include "EpaDebugSmokeEpaVmHost.h"

#include <string.h>

namespace elara {

EpaDebugSmokeEpaVmHost::EpaDebugSmokeEpaVmHost()
    : kernel(NULL) {
}
...
void EpaDebugSmokeEpaVmHost::setError(const char *prefix, const char *detail) {
...
bool EpaDebugSmokeEpaVmHost::create() {
...
void EpaDebugSmokeEpaVmHost::destroy() {
    if (kernel) {
...
bool EpaDebugSmokeEpaVmHost::setKernelId(const String &kernel_id) {
    char err[EPA_MAX_ERR];
    if (!kernel && !create()) {
        return false;
    }
...
bool EpaDebugSmokeEpaVmHost::loadAsmPath(const String &asm_path) {
    char err[EPA_MAX_ERR];
    if (!kernel && !create()) {
        return false;
    }
...
bool EpaDebugSmokeEpaVmHost::loadBlob(const uint8_t *blob, size_t blob_len) {
    if (!blob || !blob_len) {
...
bool EpaDebugSmokeEpaVmHost::ingressPush(uint32_t wid, const void *data, uint32_t len) {
    if (!kernel) {
...
bool EpaDebugSmokeEpaVmHost::ingressPushTagged(uint32_t wid, uint32_t tag, const void *data, uint32_t len) {
    if (!kernel) {
...
bool EpaDebugSmokeEpaVmHost::run(uint32_t max_ticks, bool debug) {
    char err[EPA_MAX_ERR];
    if (!kernel) {
...
void EpaDebugSmokeEpaVmHost::requestInterrupt() {
    if (kernel) {
        epa_kernel_request_interrupt(kernel);
    }
}

const uint8_t *EpaDebugSmokeEpaVmHost::resultData(size_t *out_len) const {
...
EpaKernel *EpaDebugSmokeEpaVmHost::rawKernel() const {
    return kernel;
}

bool EpaDebugSmokeEpaVmHost::isReady() const {
    return kernel != NULL;
}

const String &EpaDebugSmokeEpaVmHost::lastError() const {
    return error_text;
}
