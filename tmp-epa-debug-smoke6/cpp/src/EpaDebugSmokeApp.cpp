...
EpaDebugSmokeApp::EpaDebugSmokeApp(const String &value_host, int value_port)
    : host(value_host),
      port(value_port),
      peer(new ElaraUiRpcPeer()) {
}

void EpaDebugSmokeApp::buildDocument(ElaraUiDocumentBuilder &ui) {
    ui.clear();
    ui.createWindow(String("EpaDebugSmoke"), 1080, 760, String("org.elara.ui.epa-debug-smoke"));
    ...
    ui.addGridRowExact(String("app.panel"), 24);
    ui.createLabel(String("app.title"), String("EpaDebugSmoke control surface"), 18);
...
bool EpaDebugSmokeApp::loadDocument(const String &document_json) {
...
bool EpaDebugSmokeApp::printSnapshot() {
    String result_json;
    String error_code;
    String error_message;
