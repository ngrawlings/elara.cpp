...
def build() -> bytes:
    ...
    l_op5_true = s.alloc_id()
    l_op5_merge = s.alloc_id()
    ...
    l_textri_inner = s.alloc_id()
    l_textri_merge = s.alloc_id()
    ...
    s.emit(OP["Constant"], t_int, ci13, 13)
    s.emit(OP["Constant"], t_int, ci14, 14)
    s.emit(OP["Constant"], t_int, ci15, 15)
    s.emit(OP["Constant"], t_int, ci16, 16)
    s.emit(OP["Constant"], t_int, ci17, 17)
    s.emit(OP["Constant"], t_int, ci18, 18)
    ...
    s.emit(OP["Constant"], t_float, cf000001, fword(0.000001))
    s.emit(OP["Constant"], t_float, cfneg000001, fword(-0.000001))
    s.emit(OP["Constant"], t_float, cf05, fword(0.5))
    ...
    base_mul = tid()
    s.emit(OP["IMul"], t_int, base_mul, phi_i, ci18)
    base = tid()
    ...
    cmd_depth_mode = load_float_at(ci8)
    cmd_r = load_float_at(ci9)
    cmd_g = load_float_at(ci10)
    cmd_b = load_float_at(ci11)
    cmd_u0 = load_float_at(ci12)
    cmd_v0 = load_float_at(ci13)
    cmd_u1 = load_float_at(ci14)
    cmd_v1 = load_float_at(ci15)
    cmd_u2 = load_float_at(ci16)
    cmd_v2 = load_float_at(ci17)
    ...
    tri_denom_a = tid()
    tri_denom = tid()
    s.emit(OP["FAdd"], t_float, tri_denom_a, e0, e1)
    s.emit(OP["FAdd"], t_float, tri_denom, tri_denom_a, e2)
    tri_denom_pos = tid()
    tri_denom_neg = tid()
    tri_denom_ok = tid()
    safe_tri_denom = tid()
    s.emit(OP["FOrdGreaterThanEqual"], t_bool, tri_denom_pos, tri_denom, cf000001)
    s.emit(OP["FOrdLessThanEqual"], t_bool, tri_denom_neg, tri_denom, cfneg000001)
    s.emit(OP["LogicalOr"], t_bool, tri_denom_ok, tri_denom_pos, tri_denom_neg)
    s.emit(OP["Select"], t_float, safe_tri_denom, tri_denom_ok, tri_denom, cf1)
    tri_d0 = tid()
    tri_d1 = tid()
    tri_d2 = tid()
    s.emit(OP["FMul"], t_float, tri_d0, e0, cmd_u0)
    s.emit(OP["FMul"], t_float, tri_d1, e1, cmd_u1)
    s.emit(OP["FMul"], t_float, tri_d2, e2, cmd_u2)
    tri_d01 = tid()
    tri_d_num = tid()
    tri_interp_depth = tid()
    s.emit(OP["FAdd"], t_float, tri_d01, tri_d0, tri_d1)
    s.emit(OP["FAdd"], t_float, tri_d_num, tri_d01, tri_d2)
    s.emit(OP["FDiv"], t_float, tri_interp_depth, tri_d_num, safe_tri_denom)
    tri_uses_interp = tid()
    s.emit(OP["FOrdGreaterThanEqual"], t_bool, tri_uses_interp, cmd_depth_mode, cf05)
    tri_test_depth = tid()
    s.emit(OP["Select"], t_float, tri_test_depth, tri_uses_interp, tri_interp_depth, cmd_depth)

    depth_pass = tid()
    s.emit(OP["FOrdGreaterThanEqual"], t_bool, depth_pass, tri_test_depth, depth_after2)
    tri_write = tid()
    ...
    s.emit(OP["Phi"], t_float, depth_tri, tri_test_depth, l_tri_inner, depth_after2, l_op3_true)
    ...
    cb_after4 = tid()
    depth_after4 = tid()
    s.emit(OP["Phi"], t_float, cr_after4, cr_tex, l_tex_merge, cr_after3, l_op3_merge)
    s.emit(OP["Phi"], t_float, cg_after4, cg_tex, l_tex_merge, cg_after3, l_op3_merge)
    s.emit(OP["Phi"], t_float, cb_after4, cb_tex, l_tex_merge, cb_after3, l_op3_merge)
    s.emit(OP["Phi"], t_float, depth_after4, depth_tex, l_tex_merge, depth_after3, l_op3_merge)

    cond_op5 = tid()
    s.emit(OP["IEqual"], t_bool, cond_op5, op_i, ci5)
    s.emit(OP["SelectionMerge"], l_op5_merge, 0)
    s.emit(OP["BranchConditional"], cond_op5, l_op5_true, l_op5_merge)

    s.emit(OP["Label"], l_op5_true)
    te0a = tid()
    te0b = tid()
    te0 = tid()
    s.emit(OP["FSub"], t_float, te0a, vx, x1)
    s.emit(OP["FSub"], t_float, te0b, y2, y1)
    te0c = tid()
    te0d = tid()
    te0e = tid()
    s.emit(OP["FMul"], t_float, te0c, te0a, te0b)
    s.emit(OP["FSub"], t_float, te0d, vy, y1)
    s.emit(OP["FSub"], t_float, te0e, x2, x1)
    te0f = tid()
    s.emit(OP["FMul"], t_float, te0f, te0d, te0e)
    s.emit(OP["FSub"], t_float, te0, te0c, te0f)

    te1a = tid()
    te1b = tid()
    te1 = tid()
    s.emit(OP["FSub"], t_float, te1a, vx, x2)
    s.emit(OP["FSub"], t_float, te1b, y0, y2)
    te1c = tid()
    te1d = tid()
    te1e = tid()
    s.emit(OP["FMul"], t_float, te1c, te1a, te1b)
    s.emit(OP["FSub"], t_float, te1d, vy, y2)
    s.emit(OP["FSub"], t_float, te1e, x0, x2)
    te1f = tid()
    s.emit(OP["FMul"], t_float, te1f, te1d, te1e)
    s.emit(OP["FSub"], t_float, te1, te1c, te1f)

    te2a = tid()
    te2b = tid()
    te2 = tid()
    s.emit(OP["FSub"], t_float, te2a, vx, x0)
    s.emit(OP["FSub"], t_float, te2b, y1, y0)
    te2c = tid()
    te2d = tid()
    te2e = tid()
    s.emit(OP["FMul"], t_float, te2c, te2a, te2b)
    s.emit(OP["FSub"], t_float, te2d, vy, y0)
    s.emit(OP["FSub"], t_float, te2e, x1, x0)
    te2f = tid()
    s.emit(OP["FMul"], t_float, te2f, te2d, te2e)
    s.emit(OP["FSub"], t_float, te2, te2c, te2f)

    te0_pos = tid()
    te1_pos = tid()
    te2_pos = tid()
    s.emit(OP["FOrdGreaterThanEqual"], t_bool, te0_pos, te0, cf0)
    s.emit(OP["FOrdGreaterThanEqual"], t_bool, te1_pos, te1, cf0)
    s.emit(OP["FOrdGreaterThanEqual"], t_bool, te2_pos, te2, cf0)
    ttri_all_pos_a = tid()
    ttri_all_pos = tid()
    s.emit(OP["LogicalAnd"], t_bool, ttri_all_pos_a, te0_pos, te1_pos)
    s.emit(OP["LogicalAnd"], t_bool, ttri_all_pos, ttri_all_pos_a, te2_pos)

    te0_neg = tid()
    te1_neg = tid()
    te2_neg = tid()
    s.emit(OP["FOrdLessThanEqual"], t_bool, te0_neg, te0, cf0)
    s.emit(OP["FOrdLessThanEqual"], t_bool, te1_neg, te1, cf0)
    s.emit(OP["FOrdLessThanEqual"], t_bool, te2_neg, te2, cf0)
    ttri_all_neg_a = tid()
    ttri_all_neg = tid()
    s.emit(OP["LogicalAnd"], t_bool, ttri_all_neg_a, te0_neg, te1_neg)
    s.emit(OP["LogicalAnd"], t_bool, ttri_all_neg, ttri_all_neg_a, te2_neg)

    ttri_hit = tid()
    s.emit(OP["LogicalOr"], t_bool, ttri_hit, ttri_all_pos, ttri_all_neg)

    td0a = tid()
    td0b = tid()
    td0 = tid()
    s.emit(OP["FSub"], t_float, td0a, y1, y2)
    s.emit(OP["FSub"], t_float, td0b, x0, x2)
    s.emit(OP["FMul"], t_float, td0, td0a, td0b)
    td1a = tid()
    td1b = tid()
    td1 = tid()
    s.emit(OP["FSub"], t_float, td1a, x2, x1)
    s.emit(OP["FSub"], t_float, td1b, y0, y2)
    s.emit(OP["FMul"], t_float, td1, td1a, td1b)
    tri_denom = tid()
    s.emit(OP["FAdd"], t_float, tri_denom, td0, td1)
    ...
    tw0n1 = tid()
    tw0_num = tid()
    s.emit(OP["FSub"], t_float, tw0a, y1, y2)
    s.emit(OP["FSub"], t_float, tw0b, vx, x2)
    s.emit(OP["FSub"], t_float, tw0c, x2, x1)
    s.emit(OP["FSub"], t_float, tw0d, vy, y2)
    s.emit(OP["FMul"], t_float, tw0n0, tw0a, tw0b)
    s.emit(OP["FMul"], t_float, tw0n1, tw0c, tw0d)
    s.emit(OP["FAdd"], t_float, tw0_num, tw0n0, tw0n1)
    tw0 = tid()
    s.emit(OP["FDiv"], t_float, tw0, tw0_num, tri_denom)
    ...
    tw1n1 = tid()
    tw1_num = tid()
    s.emit(OP["FSub"], t_float, tw1a, y2, y0)
    s.emit(OP["FSub"], t_float, tw1b, vx, x2)
    s.emit(OP["FSub"], t_float, tw1c, x0, x2)
    s.emit(OP["FSub"], t_float, tw1d, vy, y2)
    s.emit(OP["FMul"], t_float, tw1n0, tw1a, tw1b)
    s.emit(OP["FMul"], t_float, tw1n1, tw1c, tw1d)
    s.emit(OP["FAdd"], t_float, tw1_num, tw1n0, tw1n1)
    tw1 = tid()
    s.emit(OP["FDiv"], t_float, tw1, tw1_num, tri_denom)

    tw0_tw1 = tid()
    tw2 = tid()
    s.emit(OP["FAdd"], t_float, tw0_tw1, tw0, tw1)
    s.emit(OP["FSub"], t_float, tw2, cf1, tw0_tw1)
    ...
    tex_u = tid()
    tex_v = tid()
    s.emit(OP["FMul"], t_float, tu0m, cmd_u0, tw0)
    s.emit(OP["FMul"], t_float, tu1m, cmd_u1, tw1)
    s.emit(OP["FMul"], t_float, tu2m, cmd_u2, tw2)
    s.emit(OP["FMul"], t_float, tv0m, cmd_v0, tw0)
    s.emit(OP["FMul"], t_float, tv1m, cmd_v1, tw1)
    s.emit(OP["FMul"], t_float, tv2m, cmd_v2, tw2)
    s.emit(OP["FAdd"], t_float, tu01, tu0m, tu1m)
    s.emit(OP["FAdd"], t_float, tv01, tv0m, tv1m)
    s.emit(OP["FAdd"], t_float, tex_u_raw, tu01, tu2m)
    s.emit(OP["FAdd"], t_float, tex_v_raw, tv01, tv2m)
    s.emit(OP["ExtInst"], t_float, tex_u, glsl_ext, GLSL["FClamp"], tex_u_raw, cf0, cf0999)
    s.emit(OP["ExtInst"], t_float, tex_v, glsl_ext, GLSL["FClamp"], tex_v_raw, cf0, cf0999)
    tex5_fx = tid()
    tex5_fy = tid()
    tex5_ix_u = tid()
    tex5_iy_u = tid()
    tex5_ix = tid()
    tex5_iy = tid()
    s.emit(OP["FMul"], t_float, tex5_fx, tex_u, tex_w_f)
    s.emit(OP["FMul"], t_float, tex5_fy, tex_v, tex_h_f)
    s.emit(OP["ConvertFToU"], t_uint, tex5_ix_u, tex5_fx)
    s.emit(OP["ConvertFToU"], t_uint, tex5_iy_u, tex5_fy)
    s.emit(OP["Bitcast"], t_int, tex5_ix, tex5_ix_u)
    s.emit(OP["Bitcast"], t_int, tex5_iy, tex5_iy_u)
    tex5_row = tid()
    tex5_flat = tid()
    tex5_off3a = tid()
    tex5_off3 = tid()
    ...
    tex5_g_idx = tid()
    tex5_b_idx = tid()
    s.emit(OP["IMul"], t_int, tex5_row, tex5_iy, tex_width)
    s.emit(OP["IAdd"], t_int, tex5_flat, tex5_row, tex5_ix)
    s.emit(OP["IMul"], t_int, tex5_off3a, tex5_flat, ci3)
    s.emit(OP["IAdd"], t_int, tex5_off3, tex5_off3a, ci2)
    s.emit(OP["IAdd"], t_int, tex5_r_idx, tex5_off3, ci0)
    s.emit(OP["IAdd"], t_int, tex5_g_idx, tex5_off3, ci1)
    s.emit(OP["IAdd"], t_int, tex5_b_idx, tex5_off3, ci2)
    tex5_r = load_tex_float(tex5_r_idx)
    tex5_g = load_tex_float(tex5_g_idx)
    tex5_b = load_tex_float(tex5_b_idx)

    tex_tri_depth_pass = tid()
    tex_tri_write = tid()
    s.emit(OP["FOrdGreaterThanEqual"], t_bool, tex_tri_depth_pass, cmd_depth, depth_after4)
    s.emit(OP["LogicalAnd"], t_bool, tex_tri_write, ttri_hit, tex_tri_depth_pass)
    s.emit(OP["SelectionMerge"], l_textri_merge, 0)
    s.emit(OP["BranchConditional"], tex_tri_write, l_textri_inner, l_textri_merge)
    s.emit(OP["Label"], l_textri_inner)
    s.emit(OP["Branch"], l_textri_merge)
    s.emit(OP["Label"], l_textri_merge)
    cr_tex_tri = tid()
    cg_tex_tri = tid()
    cb_tex_tri = tid()
    depth_tex_tri = tid()
    s.emit(OP["Phi"], t_float, cr_tex_tri, tex5_r, l_textri_inner, cr_after4, l_op5_true)
    s.emit(OP["Phi"], t_float, cg_tex_tri, tex5_g, l_textri_inner, cg_after4, l_op5_true)
    s.emit(OP["Phi"], t_float, cb_tex_tri, tex5_b, l_textri_inner, cb_after4, l_op5_true)
    s.emit(OP["Phi"], t_float, depth_tex_tri, cmd_depth, l_textri_inner, depth_after4, l_op5_true)
    s.emit(OP["Branch"], l_op5_merge)

    s.emit(OP["Label"], l_op5_merge)
    cr_new = tid()
    cg_new = tid()
    cb_new = tid()
    depth_new = tid()
    s.emit(OP["Phi"], t_float, cr_new, cr_tex_tri, l_textri_merge, cr_after4, l_op4_merge)
    s.emit(OP["Phi"], t_float, cg_new, cg_tex_tri, l_textri_merge, cg_after4, l_op4_merge)
    s.emit(OP["Phi"], t_float, cb_new, cb_tex_tri, l_textri_merge, cb_after4, l_op4_merge)
    s.emit(OP["Phi"], t_float, depth_new, depth_tex_tri, l_textri_merge, depth_after4, l_op4_merge)
